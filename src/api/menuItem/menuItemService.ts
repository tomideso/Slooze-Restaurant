...
export class MenuItemService {
  ...
  async findAll(): Promise<ServiceResponse<MenuItem[] | null>> {
    try {
      ...
      return ServiceResponse.failure("An error occurred while retrieving menu items.", null, StatusCodes.INTERNAL_SERVER_ERROR);
    }
  }
  ...
  async findById(id: MenuItem["_id"]): Promise<ServiceResponse<MenuItem | null>> {
    try {
      ...
      logger.error(errorMessage);
      return ServiceResponse.failure("An error occurred while finding menu item.", null, StatusCodes.INTERNAL_SERVER_ERROR);
    }
  }
  ...
  async findByRestaurantId(restaurantId: Restaurant["_id"]): Promise<ServiceResponse<MenuItem[] | null>> {
    try {
      ...
      logger.error(errorMessage);
      return ServiceResponse.failure("An error occurred while finding menu items.", null, StatusCodes.INTERNAL_SERVER_ERROR);
    }
  }

  // Creates a new menu item
  async create(menuItemData: Omit<Partial<MenuItem>, "id" | "createdAt" | "updatedAt">): Promise<ServiceResponse<MenuItem | null>> {
    try {
      ...
      logger.error(errorMessage);
      return ServiceResponse.failure("An error occurred while creating menu item.", null, StatusCodes.INTERNAL_SERVER_ERROR);
    }
  }
  ...
  async update(id: MenuItem["_id"], menuItemData: Partial<MenuItem>): Promise<ServiceResponse<MenuItem | null>> {
    try {
      ...
      const updatedMenuItem = await this.menuItemRepository.findByIdAndUpdate(id, { $set: menuItemData }, { new: true });
      if (!updatedMenuItem) {
        return ServiceResponse.failure("Menu Item not found", null, StatusCodes.NOT_FOUND);
      }
      ...
      logger.error(errorMessage);
      return ServiceResponse.failure("An error occurred while updating menu item.", null, StatusCodes.INTERNAL_SERVER_ERROR);
    }
  }
  ...
  async delete(id: MenuItem["_id"]): Promise<ServiceResponse<null>> {
    try {
      ...
      logger.error(errorMessage);
      return ServiceResponse.failure("An error occurred while deleting menu item.", null, StatusCodes.INTERNAL_SERVER_ERROR);
    }
  }
}
