...
export class UserService {
  ...
  public async register(user: User): Promise<ServiceResponse<User | null>> {
    try {
      ...
      return ServiceResponse.success<User>("User Registered Successfully", newUser);
    } catch (error) {
      const errorMessage = `Error finding all users: $${(error as Error).message}`;
      logger.error(errorMessage);
      return ServiceResponse.failure("An error occurred while retrieving users.", null, StatusCodes.BAD_REQUEST);
    }
  }
